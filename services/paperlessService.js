...
class PaperlessService {
    ...
    async createTagSafely(tagName) {
        ...
        try {
            // 1890: add random color generation for newly tag
            const randomColor = Math.floor(Math.random() * 16777215).toString(16).padStart(6, '0');
            // Versuche zuerst, den Tag zu erstellen
            const response = await this.client.post('/tags/', { name: tagName, color: `#${randomColor}` });
            const newTag = response.data;
            console.log(`[DEBUG] Successfully created tag "${tagName}" with ID ${newTag.id} and color #${randomColor}`);
