...
const fs = require('fs'); // Aggiunto per leggere il file
const currentDir = decodeURIComponent(process.cwd());

// 1890: Leggiamo il file VERSION nella root del progetto
let versionFromFile = '0.0.0-1890.1.0.0.default'; // Fallback
try {
    const versionPath = path.join(currentDir, 'VERSION');
    if (fs.existsSync(versionPath)) {
        versionFromFile = fs.readFileSync(versionPath, 'utf8').trim();
    }
} catch (err) {
    // In caso di errore resta 3.0.9
...
module.exports = {
    PAPERLESS_AI_VERSION: versionFromFile, // 1890: version from file...
    CONFIGURED: false,
